...
impl IndradbClientBackend {
    ...
    async fn create_emu_net(&self, user: String, net: String, capacity: u32) -> Result<QueryResult<Uuid>, BackendError> {
        ...
        let emu_net_id = self.create_vertex(None).await?.expect("vertex ID already exists");
        ...
        let res = self.set_vertex_json_value(emu_net_id, "default", &jv).await?;
        if !res {
    ...
    async fn list_emu_net(&self, user: String) -> Result<QueryResult<HashMap<String, Uuid>>, BackendError> {
        // get user
        let user_map: HashMap<String, user::EmuNetUser> = self.get_core_property("user_map").await?;
    ...
    async fn get_emu_net(&self, uuid: Uuid) -> Result<QueryResult<net::EmuNet>, BackendError> {
        let res = self.get_vertex_json_value(uuid, "default").await?;
        match res {
            None => Ok(QueryFail("EmuNet not exist".to_string())),
    ...
    async fn set_emu_net(&self, emu_net: net::EmuNet) -> Result<QueryResult<()>, BackendError> {
        ...
        let jv = serde_json::to_value(emu_net).unwrap();
        let res = self.set_vertex_json_value(uuid, "default", &jv).await?;
        match res {
...
impl IndradbClientBackend {
    async fn dispatch_request(&self, req: Request) -> Result<Response, BackendError> {
        match req {
            ...
            Request::ListEmuNetUuid(user) => {
                self.list_emu_net(user).await.map(|hmap|{Response::ListEmuNetUuid(hmap)})
            },
