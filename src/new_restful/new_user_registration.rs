...
) -> impl Filter<Extract = (), Error = warp::Rejection> + Clone + Send + Sync {
    let res = warp::post()
        .and(warp::path("v1"))
...
fn parse_json<T: serde::de::DeserializeOwned + Send>(
    prefix_filter: impl Filter<Extract = (), Error = warp::Rejection> + Clone + Send + Sync,
) -> impl Filter<Extract = (T,), Error = warp::Rejection> + Clone + Send + Sync {
    let res = prefix_filter.and(super::parse_json_body());
    res
}

// fn get_client<T: serde::de::DeserializeOwned + Send>(
//     json_filter: impl Filter<Extract = (T,), Error = warp::Rejection> + Clone + Send + Sync,
//     connector: Connector,
// ) -> impl Filter<Extract = (T, Client), Error = warp::Rejection> + Clone + Send + Sync {
//     let res = 
//     json_filter.and(move |(t,)| async {
