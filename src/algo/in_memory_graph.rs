...
use super::traits::{Partition, PartitionBin};
...
{
    pub fn from_vecs(
        vertexes: Vec<(Vid, Vertex)>,
        edges: Vec<(EdgeId<Vid>, Edge)>,
    ) -> Result<Self> {
        if vertexes.len() == 0 {
        ...
        let insert_res: Result<Vec<_>> = vertexes
            .into_iter()
            .map(|(vid, v)| {
        ...
        let insert_res: Result<Vec<_>> = edges
            .into_iter()
            .map(|(eid, e)| {
